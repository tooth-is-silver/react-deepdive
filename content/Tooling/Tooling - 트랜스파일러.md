...
트랜스파일러가 여럿으로 나뉘는 건 세 축에서 갈리기 때문입니다. 첫째는 하는 일의 범위로, 순수 변환만 하는 것(Babel·SWC·Sucrase), 타입 검사까지 하는 것(tsc), 번들링까지 겸하는 것(esbuild)으로 나뉩니다. 둘째는 구현 언어에 따른 속도로, JS 기반 Babel보다 Rust 기반 SWC와 Go 기반 esbuild가 훨씬 빠릅니다. 셋째는 타입 검사 여부인데, tsc를 뺀 대부분은 타입을 검사하지 않고 그냥 지우기(type stripping)만 해서 빠른 대신 타입 에러를 못 잡습니다. 그래서 실무에서는 변환은 빠른 SWC·esbuild가 맡고 타입 검사는 `tsc --noEmit`로 따로 돌리는 식으로 역할을 나눕니다.
...
- 최신 문법 변환
- 번들러와 차이
- 타입 검사는 tsc만(나머지는 type stripping)
- 구현 언어별 속도(JS/Rust/Go)
